...
<info>php bolt-site-manager.phar %command.name%</info>
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($this->getApplication()->getVersion() === $this->buildVersionString($major, $minor, $patch)) {
            $output->writeln('<info>bolt-site-manager is already up to date.</info>');
        ...
        try {
            ...
            $output->writeln('<info>bolt-site-manager updated.</info>');
        } catch (\Exception $e) {
    ...
    {
        $version = $this->buildVersionString($major, $minor, $patch);
        
        return sprintf('https://github.com/bolt/bolt-site-manager/v%s/bolt-site-manager-v%s.phar', $version, $version);
    }
